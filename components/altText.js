...
export const checkAltText = (document) => {
  try {
    ...
    images.forEach((element, index) => {
      ...
      const hasDecorativeClass = (element.classList.contains('decorative') && element.getAttribute('alt') === '');

      if (!alt && !isHidden && !hasDecorativeClass && !displayNone) {
        output = output + `${chalk.red('\nImage without appropriate alt text:')}${chalk.cyan(element.outerHTML)}`;
      }
    });
    if (output.length === 0) {
