...
export const checkReducedMotion = async () => {
  try {
    ...
    if (animations.length == 0 || mediaQueryPattern.test(cssContent)) {
      return chalk.green(
        "All animations obey the prefers-reduced-motion media query",
      );
    } else {
      return chalk.red(
        "Animations do not obey the prefers-reduced-motion query",
      );
    }
  } catch (err) {
    return `${chalk.red("Error processing the document or CSS file:")}${err}`;
...
function countKeyframes(cssCode, animationName) {
  const keyframesPattern = new RegExp(
    `@keyframes\\s+${animationName}\\s*{([^}]*)}`,
    "g",
  );
...
function checkFlashes(cssContent) {
  ...
  const violations = animations
    .filter(
      (animation) => animation.frames / animation.duration > flashesPerSec,
    )
