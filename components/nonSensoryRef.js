...
export const checkNonSensoryRef = (document) => {
  try {
    ...
    const sensoryCharacteristics = ["shaped", "colored", "bottom", "beside", "top"];

    const hasNonSensoryInformation = (reference) => {
      return !sensoryCharacteristics.some(char => reference.includes(char));
    };

    const references = document.querySelectorAll('a, button, [role="button"], [role="link"], input');
    ...
    references.forEach((element, index) => {
      ...
      if (!hasNonSensoryInformation(text)) {
        output += `${chalk.red('\nReference relies on sensory characteristics:')}${chalk.cyan(element.outerHTML)}`;
      }
    });
